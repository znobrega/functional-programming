...
(define (subconjunto? c1 c2)
  (cond [(empty? c1) #t]
        [(pertence? (first c1) c2) (subconjunto? (rest c1) c2)]
        [else #f]))
...
(define (remove-duplicatas lst)
  (cond [(empty? lst) '()]
        [(pertence? (first lst) (rest lst))
                     (remove-duplicatas (rest lst))]
        [else (cons (first lst) (remove-duplicatas (rest lst)))]))
...
(define (diferenca c1 c2)
  (cond [(empty? c1) '()]
        [(empty? c2) '()]
        [(pertence? (first c1) c2) (diferenca (rest c1) c2)]
